...
class FoodDeliveryExtendedTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_view_single_order(): void
    {
        ...
        $response = $this->actingAs($this->customer)
            ->getJson("/api/v1/food/orders/{$order->id}");
    ...
    public function test_rider_cannot_access_driver_routes(): void
    {
    ...
    {
        $this->getJson('/api/v1/food/orders')->assertStatus(401);
        $this->postJson('/api/v1/food/restaurants/1/order', [])->assertStatus(401);
    }
}
